...
export default function DiscoverServersModal({ isOpen, embedded, onClose, onServerJoined, onBack, exiting }) {
  ...
  const { t } = useLanguage();
  const enterInstant = useModalEnterAnimation('discover-servers-modal', isOpen && !embedded);
  ...
  if (embedded) {
    return <div className="dsm-embedded">{content}</div>;
  }
